      ...
      step={{
        ...
        children: (
          <>
              ...
              <div className="flex flex-col gap-2">
                ...
                {isLoadingBlog && (
                  <div className="alert alert-info">
                    ...
                    The Siren is crafting your content...
                  </div>
                )}
              </div>
            )}

            {/* Show streaming content as it loads */}
            {isLoadingBlog && blogContent && (
            ...
            {/* Show final portable text content when complete */}
            {portableTextContent?.length > 0 && !isLoadingBlog && (
              <PortableTextRenderer content={portableTextContent} />
            )}
            
            {/* Fallback to HTML rendering if no portable text is available and not loading */}
            {!portableTextContent?.length && blogContent && !isLoadingBlog && (
              <div className="w-full max-w-3xl mx-auto bg-white dark:bg-base-300 rounded-xl shadow-lg border border-base-300 p-8">
